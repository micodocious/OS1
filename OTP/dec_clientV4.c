...
main(int argc, char *argv[])
{
  ...
  //s = getaddrinfo(NULL, argv[3], &hints, &result);
  ...
  // confirm if dec_client -> dec_server
  char confirm_dec_client[4];
  memset(confirm_dec_client,'\0',2);
  charsWritten=recv(sfd,confirm_dec_client,3,0);
  if(strcmp(confirm_dec_client,"dec")!=0){
