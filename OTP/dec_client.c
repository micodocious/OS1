 ...
 * Nov 29, 2022
 *
 * This program decrypts the output of enc_client using the ciphertext and the key
 * used by enc_client and produced by our keygen.
 * To use, compile using gcc -o dec_client dec_client.c and run using
 * $./dec_client [FILE TO BE ENCRYPTED] [KEY MADE BY KEYGEN] [PORT] 
 */
...
main(int argc, char *argv[])
{
  ...
  if(flag==1){
    fprintf(stderr, "DEC_CLIENT: Contains bad characters!\n");
    exit(1);
  }
  ...
  if(strcmp(confirm_enc_client,"dec")!=0){
    fprintf(stderr, "DEC_CLIENT: Incorrect Server.\n");
    exit(2);
  }
  ...
  if (charsWritten < 0){
    fprintf(stderr,"DEC_CLIENT: ERROR writing text length to socket");
  }
  // send key length to server 
  charsWritten = send(sfd, &key_length, sizeof(int), 0); 
  if (charsWritten < 0){
    fprintf(stderr,"DEC_CLIENT: ERROR writing key length to socket");
  }
  ...
  if (charsWritten < 0){
    fprintf(stderr,"DEC_CLIENT: ERROR writing text buffer to socket");
  }
  ...
  if (charsWritten < 0){
    fprintf(stderr,"DEC_CLIENT: ERROR writing key buffer to socket");
  }
