...
void *line_seperator_thread(void *args)
{
  ...
  for (int i = 0; i < NUM_ITEMS; i++)
  {
    ...
    char *itemTemp = itemPtr;
    size_t newLineLen = strlen(newLine);
    size_t spaceLength = strlen(space);

    while(1)
    {
      char *temp = strstr(itemTemp, newLine);
      if (temp == NULL)
      {
        strcpy(bufferPtr, itemTemp);
        break;
      }

      int temptemp = (temp - itemTemp);
      memcpy(bufferPtr, itemTemp, temp - itemTemp);
      bufferPtr = bufferPtr + temptemp;

      memcpy(bufferPtr, space, spaceLength);
      bufferPtr = bufferPtr + spaceLength;

      itemTemp = temp + newLineLen;

    }
    strcpy(itemPtr, buffer);
    put_buff_2(item);
  }
  return NULL;
}
...
void *plus_sign_thread(void *args)
{
  ...
  for (int i = 0; i < NUM_ITEMS; i++)
  {
    ...
    char *bufferPtr = &buffer[0];
    char *itemTemp = itemPtr;
    size_t plusplusLen = strlen(plusplus);
    size_t carrotLen = strlen(carrot);

    while(1)
    {
      char *temp = strstr(itemTemp, plusplus);
      if (temp == NULL)
      {
        strcpy(bufferPtr, itemTemp);
        break;
      ...
      memcpy(bufferPtr, carrot, carrotLen);
      bufferPtr = bufferPtr + carrotLen;

      itemTemp = temp + plusplusLen;
...
void *write_output(void *args){
  ...
  fflush(stdout);
   while((strlen(final) - j) >= 80) // while the next possible print is still greater than or equal to 80 characters
      {
        k = j;
        j += 80;
        for(; k < j; k++)
        {
          printf("%c", final[k]); // print 80 characters at a time
          fflush(stdout);
        }
        printf("\n");
        fflush(stdout);
 ...
 int count=0;
 for(int line=1;(strlen(final)/line)>=80;){ 
  for(int i=0;i<=80;i++){
      printf("%c", final[i]);
      fflush(stdout);
  }
  ...
  for(int i=81;i<=80*2+1;i++){
      printf("%c", final[i]);
  }
  printf("\n");
  for(int i=81*2;i<=80*3+2;i++){
      printf("%c", final[i]);
  }
  printf("\n");
  for(int i=81*3;i<=80*4+3;i++){
      printf("%c", final[i]);
  }
...
void *write_output(void *args)
{
  ...
  int currLine = 0;
    ...
    {
      get_buff_3(item);
      if(strcmp(item, "STOP\n") == 0) // when we see "STOP", we stop printing
      {
        break;
      }
      if(start == 0)
      {
        snprintf(itemBuffer, sizeof(itemBuffer), "%s", item);
        start = 1;
        fflush(stdout);
      }
      else
      {
        strcpy(temp, itemBuffer);
        snprintf(itemBuffer, sizeof(itemBuffer), "%s%s", temp, item);
        fflush(stdout);
      }
      //int currLine = 0;
      int j;
      while((strlen(itemBuffer) - currLine) >= 80) //does the next line have more or equal to 80 charcs?
      {
        j = currLine;
        currLine = currLine + 80;
        for(; j < currLine; j++)     
        {
          printf("%c", itemBuffer[j]); //prints the line
          fflush(stdout);
        }
        printf("\n");
        fflush(stdout);
