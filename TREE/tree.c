/* Mico Santiago
 * Oct 19. 2022
 * Operating Systems 1 w/ Ryan Gambord
 * Assignment 2 - TREE
 */
...
print_tree(char const *path)
{
  ...
  if(opts->dirsonly==true && S_ISDIR(st.st_mode)){
    for(int i=0;i<indent_level;i++){printf("  ");}
    print_path_info(path,st);
  } 
  else if(opts->dirsonly==true && !S_ISDIR(st.st_mode)){
    goto exit;
  } 
  else{for(int i=0;i<indent_level;i++){printf("  ");}
    print_path_info(path,st);
  }
   ...
   *   We also check for directories which can't be opened*/
...
print_path_info(char const *path, struct stat st)
{
  ...
  if (opts->perms) {
     ...
     * It's documented in the cs344/libtree_testben print_path_info() - Helper function
     * You might want to look at it... just a suggestion :)  */
    mode_t permissions=st.st_mode;
    const char* var=modestr(permissions);
    printf("%c%s", sep, var); 
    sep = ' ';
  }
  if (opts->user) {
    ...
    printf("%c%s", sep, pwd->pw_name); 
    sep = ' ';
  }
  if (opts->group) {
    /*  Hint: getgrgid(3) */
    struct group* grp=getgrgid(st.st_gid);
    printf("%c%s", sep, grp->gr_name); 
    sep = ' ';
  }
  if (opts->size) {
    /*  Hint: stat.h(0p) */
    printf("%c%zd", sep, st.st_size); 
    sep = ' ';
  }
  if (sep != '[') printf("] ");
  printf("%s", path);
...
filesort(void const *_lhs, void const *_rhs)
{
  ...
  switch (opts->sort)
  {
    ...
    case RALPHA:
      return strcoll(rhs->name, lhs->name);
    case TIME:
